...
class StudentQuestionProgress extends Model
{
    ...
    {
        return [
            [['student_id', 'question_id'], 'required'],
            [['student_id', 'question_id', 'progress'], 'integer'],
    ...
    public function updateProgress()
    {
        ...
        $model = Progress::findProgressModel($this->student_id, $this->question_id);
        if ($model !== null) {
            $model->updateProgress($this->progress);
        }
        else {
            $model = Progress::create($this->student_id, $this->question_id, $this->progress);
        }
        $model->save();
    }
