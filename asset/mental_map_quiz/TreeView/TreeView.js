...
`
  ...
  header.querySelector('#clear-history').addEventListener('click', async () => {
    ...
    const {
      ...
      presentationHistory: emptyPresentationHistory
    } = await restartMentalMap(id)
    if (success) {

      history.length = 0
      emptyHistory.map(h => history.push(h))

      if (presentationHistory) {
        presentationHistory.length = 0
        emptyPresentationHistory.map(h => presentationHistory.push(h))
      }

      if (hiddenMapProgress) {
        hiddenMapProgress.reset()
      }

      if (allMapProgress) {
        allMapProgress.reset()
      }

      wrap.appendChild(body.restart())
      header.querySelector('#clear-history').style.display = 'none'
      onMentalMapChange(0)
    }
  })
