...
import {calcHiddenTextPercent} from "../MentalMap";

export default function MentalMapQuestions({typeIsMentalMapQuestions, questions}) {
  this.typeIsMentalMapQuestions = () => Boolean(typeIsMentalMapQuestions)
  this.findQuestion = (id) => (questions || []).find(q => q.fragmentId === id)
}

function FragmentResultElement(historyItem) {
...
export function mapImageClickHandlerQuestions({image, questionItem}) {
...
`
  ...
  detailImgWrap.appendChild(FragmentResultElement())

  const detailTextWrap = document.createElement('div')
  detailTextWrap.classList.add('detail-text-wrap')

  const detailText = DetailTextQuestions(questionItem?.questions)
