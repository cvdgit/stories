...
function init(payload, container, config) {
  ...
  container.parents('.slides').find('section').each((i, el) => {
    const $el = $(el);
...
TableOfContents.initDeckEvent = function (deck, config) {
  ...
  if ($(deck.getCurrentSlide()).find(`[data-block-type='table-of-contents']`).length > 0) {
    ...
    init(
      payload,
      $(deck.getCurrentSlide()).find('.table-of-contents'),
      config
    );
    return;
  }

  const $prevTableOfContentsSlide = $(deck.getCurrentSlide()).prevAll(`[data-slide-view='table-of-contents']:eq(0)`);
